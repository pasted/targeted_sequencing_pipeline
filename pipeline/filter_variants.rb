class FilterVariants
	...
	def annotate_filters(sample, batch, logger)
			...
			input_file_string="../../variants/haplotyper/#{sample.panel_version.downcase}_#{sample.sample_id}_#{sample.gender.upcase}_#{sample.phenotype.upcase}.hap_call.vcf"
			output_file_string="../../variants/haplotyper/#{sample.panel_version.downcase}_#{sample.sample_id}_#{sample.gender.upcase}_#{sample.phenotype.upcase}.filtered.hap_call.vcf"
			...
			cmd += "-o #{output_file_string} --variant #{input_file_string} -log ../../logs/#{sample.panel_version}_#{sample.sample_id}_filtered_hap_call.log"
	...
	def annotate_snp_filters(input_file_string, output_file_string, batch, logger)
			...
			cmd = "#{batch.java_path} -Xmx4g -jar #{batch.gatk_path} -T VariantFiltration -R #{batch.reference_path} --filterExpression 'QD < 2.0' --filterName 'QD2' --filterExpression 'MQ < 40.0' --filterName 'MQ40' "
			cmd += "--filterExpression 'ReadPosRankSum < -8.0' --filterName 'RPRS-8' --filterExpression 'FS > 60.0' --filterName 'FS60' --filterExpression 'MQRankSum < -12.5' --filterName 'MQRankSum-12.5' "
			cmd += "-o #{output_file_string} --variant #{input_file_string} -log ../../logs/#{batch.batch_id}_filtered_6q24_ug_call.log"
	...
	def select_discordant_variants(sample, batch, logger, prefix_in, prefix_out, discordant_file_path)
			...
			input_file_string="../../variants/haplotyper/#{sample.panel_version.downcase}_#{sample.sample_id}_#{sample.gender.upcase}_#{sample.phenotype.upcase}.#{prefix_in}.hap_call.vcf"
			output_file_string="../../variants/haplotyper/#{sample.panel_version.downcase}_#{sample.sample_id}_#{sample.gender.upcase}_#{sample.phenotype.upcase}.#{prefix_out}.hap_call.vcf"
	...
	def select_phenotype_variants(sample, batch, logger, prefix_in, prefix_out)
			...
			cmd = "#{batch.java_path} -Xmx4g -jar #{batch.gatk_path} -T SelectVariants -R #{batch.reference_path} "
			cmd += "-o #{output_file_string} --variant #{input_file_string} -L #{phenotype_intervals_path} -log ../../logs/#{sample.panel_version}_#{sample.sample_id}_#{sample.phenotype}.log"
