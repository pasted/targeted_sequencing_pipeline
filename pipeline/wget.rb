class Wget
	...
	def fetch_files(sample, parser, logger)
			...
			output = this_wrapper.run_command("wget -r -l 1 --no-parent -nH --cut-dirs=3 -A \"*.fastq.md5sum\" -P \"#{parser.base_path}/#{parser.batch_id}/raw_reads/md5sum/\" -w 1  #{parser.ftp_url}/#{parser.flowcell}/#{this_folder}", logger)
			output = this_wrapper.run_command("wget -r -l 1 --no-parent -nH --cut-dirs=3 -A \"*.fastq\" -P \"#{parser.base_path}/#{parser.batch_id}/raw_reads/\" -w 1 #{parser.ftp_url}/#{parser.flowcell}/#{this_folder}", logger)
