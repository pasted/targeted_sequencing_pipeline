class SnpParser
  ...
  def write_snps_scores(snps_hash, this_book, worksheet_name)
		...
		row_number = 0
		this_sheet.row(row_number).push "Sample Id", "Score", "Percentage"
		row_number = row_number + 1
		snps_hash.each_pair do |this_key, this_score|		
			this_sheet.row(row_number).push "#{this_key}", "#{this_score[0]}", "#{this_score[1]}"
			row_number = row_number + 1
		end
  ...
  ranked_scores = parser.load_ranked_scores("positive_control_percentages.csv")
  ...
  sample_hash.each do |this_key, this_array|
  	...
  	ranked_scores.each do |this_rank|
