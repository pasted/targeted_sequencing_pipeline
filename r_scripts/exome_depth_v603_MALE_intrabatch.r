...
#Alignment_folder <- args[1]

Alignment_folder <- '/mnt/Data4/targeted_sequencing/2016-07-04_2363/cnv_bams'
print(Alignment_folder)
setwd(Alignment_folder)
#bed <- args[2]
bed <- '/mnt/Data1/targeted_sequencing/intervals/v603/grch-37/v603_CNV_120bp.bed'

#referenceFasta <- args[3]
reference.fasta <- '/mnt/Data1/resources/human_g1k_v37.fasta'

#get list of sample BAM files
bams.selected <- Sys.glob('v603*_MALE.realigned.bam')
bais.selected <- Sys.glob('v603*_MALE.realigned.bai')
...
dir.results <- NULL
	...
	my.counts <- getBamCounts(bed.file = bed, bam.files = bams.selected, include.chr = FALSE, referenceFasta = reference.fasta)
...
#	print(samplenames)
	dir.base <- "/mnt/Data4/targeted_sequencing/2016-07-04_2363/cnv_analysis/male/v603/results"
	save.image( sprintf("%s/exome_depth_count_v603_male.RData", dir.base) )
	...
	for (i in 1:nsamples) {
            ...
            print(paste("Current sample :: ",samplename))
  #### Create the aggregate reference set for this sample
            ...
            print(paste("Reference choice ::", my.choice$reference.choice))
  ################ Now call the CNVs
